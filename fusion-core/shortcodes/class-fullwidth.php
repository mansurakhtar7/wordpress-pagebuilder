	...
	class FusionSC_FullWidth {
		...
		function render( $args, $content = '' ) {
			...
			if ( $defaults['hundred_percent'] == 'yes' ) {
				$defaults['padding-left']  = '0px';
				$defaults['padding-right'] = '0px';
			}
			...
			$html = sprintf( '%s<div %s>%s<div %s>%s</div></div>', $parallax_helper, FusionCore_Plugin::attributes( 'fullwidth-shortcode' ), $outer_html, FusionCore_Plugin::attributes( 'fusion-row' ), do_shortcode( $content ) );
