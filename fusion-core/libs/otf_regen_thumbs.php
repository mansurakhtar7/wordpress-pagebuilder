<?php
	/*
	 * Based off OTF Regenerate Thumbnails, modified to include proper fallback and upscaling
	 * by Dovy Paukstys
	...
	*/
	 ...
	 */
	if ( ! function_exists( 'avada_gambit_otf_regen_thumbs_media_downsize' ) ) {

		add_filter( 'image_downsize', 'avada_gambit_otf_regen_thumbs_media_downsize', 10, 3 );
		 ...
		 */
		function avada_gambit_otf_regen_thumbs_media_downsize( $out, $id, $size ) {
			...
			if ( is_string( $size ) ) {
				...
				$imagePath = $orig_path = get_attached_file( $id );
				...
				// Resize somehow failed
				if ( ! $resized ) {
					$imagedata['url']  = $att_url;
					$imagedata['path'] = $orig_path;
					$upscale           = avada_gambit_otf_regen_thumbs_media_upsize( $id, $imagedata, $size, $allSizes );
					return array( $upscale['url'], $upscale['width'], $upscale['height'], true );
				}
		...
		function avada_gambit_otf_regen_thumbs_media_upsize( $id, $meta, $size, $allSizes ) {

			if ( !empty( $size[0] ) && !empty( $size[1] ) ) {
				$imageExt         = pathinfo( $meta['path'], PATHINFO_EXTENSION );
				$meta['new_path'] = preg_replace( '/^(.*)\.' . $imageExt . '$/', sprintf( '$1-%sx%s.%s', $size[0], $size[1], $imageExt ), $meta['path'] );
				if ( file_exists( $meta['new_path'] ) ) {
					$editor     = wp_get_image_editor( $meta['new_path'] );
					$image_size = $editor->get_size();

					return array(
						'url'    => str_replace( basename( $meta['url'] ), basename( $meta['new_path'] ), $meta['url'] ),
						'width'  => $image_size['width'],
						'height' => $image_size['height']
					);
				}
			}
			$editor = wp_get_image_editor( $meta['path'] );
			if ( is_wp_error( $editor ) ) {
				return array( 'url' => $meta['url'], 'width' => $meta['width'], 'height' => $meta['height'] );
			}
			$image_size = $editor->get_size();
			...
			if ( empty( $size[0] ) || empty( $size[1] ) ) {
				...
				if ( ! empty( $size[0] ) ) {
					$ratio   = $size[0] / $image_size['width'];
					$size[1] = floor( $image_size['height'] * $ratio );
				} elseif ( ! empty( $size[1] ) ) {
					$ratio   = $size[0] / $image_size['height'];
					$size[0] = floor( $image_size['width'] * $ratio );
				} else {
					return array(
						'url'    => $meta['url'],
						'width'  => $image_size['width'],
			...
			$editor->crop( 0, 0, $image_size['width'], $image_size['height'], $size[0], $size[1] );

			// Now let's save the image
			$resized = $editor->save( $meta['new_path'] );

			// Get resized image information
			$resized_url    = str_replace( basename( $meta['url'] ), basename( $meta['new_path'] ), $meta['url'] );
